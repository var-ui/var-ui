...
| Priority | Astryx reference                                                        | var-ui deliverable                                                     |
| -------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| P1       | `IconButton`, `ButtonGroup`                                             | Extend button recipe variants                                          |
| P1       | `SegmentedControl`, `ToggleButton`                                      | Selection button groups                                                |
| P1       | `DropdownMenu`, `ContextMenu`, `MoreMenu`                               | Menu recipes on RAC Menu/MenuTrigger                                   |
| P2       | `Toolbar`                                                               | Horizontal action grouping                                             |
| P1       | `Slider`, `NumberInput`                                                 | RAC NumberField/Slider                                                 |
| P2       | `FileInput`                                                             | Styled native file input                                               |
| P2       | `InputGroup`, `InputGroupText`                                          | Prefix/suffix chrome                                                   |
| P2       | `CheckboxList`, `RadioList`                                             | Build on existing checkbox/radio recipes                               |
| P2       | `Calendar`, `DateInput`, `DateRangeInput`, `DateTimeInput`, `TimeInput` | RAC DatePicker/Calendar (largest single PR cluster — split by control) |
| P3       | `Typeahead`, `Tokenizer`                                                | Autocomplete and token input (depends on list focus hooks)             |
| P3       | `Selector`, `MultiSelector`                                             | Combobox patterns beyond basic Select                                  |
| P3       | `PowerSearch`                                                           | Composite filter UI (defer until Typeahead + Tokenizer land)           |
