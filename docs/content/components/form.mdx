...
Native-constraint form that focuses the first invalid field. `Form` is a React `<form>` that runs `checkValidity()`, accepts a server `errors` map keyed by field `name`, and does not replace `@var-ui/form` `useForm`.
