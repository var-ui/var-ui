...
- **Stable semantic class names are public API**: scopeId is `var-ui`, so classes read `var-ui-<component>-<slot>[-<variant>-<value>]`. Never rename an existing class. New classes must be added to `packages/core/.typestyles-public-classnames.json` (Task 21).
...
}; // renders <hr>, vertical gets role="separator" aria-orientation
...
const pulse = keyframes.create('var-ui-status-pulse', {
  ...
  to: {
    boxShadow: '0 0 0 6px color-mix(in srgb, currentColor 0%, transparent)',
  },
});
...
export function Heading({
  ...
  return createElement(`h${level}`, {
    ...props,
    className: cx(heading({ size }), className),
  });
}
    ...
    <span className={cx(a.root, className)}>
      ...
      {status ? (
        <span className={a.status} data-avatar-status>
          <span
            className={statusDot({
              tone: status === 'neutral' ? 'neutral' : status,
            })}
          />
        </span>
      ) : null}
